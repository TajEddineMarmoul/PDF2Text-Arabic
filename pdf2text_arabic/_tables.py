...
def _slice_region_into_rows(
    rawdict: dict, table, tx0: float, tx1: float
) -> list[list[str]]:
...
def extract_tables(
        ...
        if initial_tables:
            ...
            for t in initial_tables:
                ...
                mixed_tabs = page.find_tables(
                    vertical_strategy="lines", horizontal_strategy="text", clip=col_clip
                )
                ...
                if mixed_tabs.tables:
                    for mt in mixed_tabs.tables:
                        ...
                        primary_is_truncated = (clip.y1 - t.bbox[3]) > max(
                            80.0, page.rect.height * 0.15
                        )
                        min_cols = (
                            max(5, len(t.header.cells) - 2)
                            if primary_is_truncated
                            else len(t.header.cells)
                        )
                        if (
                            len(mt.header.cells) >= min_cols
                            and len(mt.rows) > len(best_t.rows) + 2
                        ):
            ...
            mixed_tabs = page.find_tables(
                vertical_strategy="lines", horizontal_strategy="text", clip=page.rect
            )
            for t in mixed_tabs.tables:
                ...
                if len(t.header.cells) >= 3:
                    if len(t.rows) >= 2 or (
                        len(t.rows) == 1 and len(t.header.cells) > 2
                    ):
                        candidates.append(t)
    else:
    ...
    for i, table in enumerate(candidates):
        ...
        if clip is not None:
            ...
            if (t_bbox.width > page.rect.width * 0.6) or (
                abs(t_bbox.x0 - clip.x0) < 100 and abs(t_bbox.x1 - clip.x1) < 100
            ):
            ...
            for ri, row in enumerate(table.rows):
                ...
                for ci, cell in enumerate(reversed(row.cells)):
                    ...
                    if cell is None:
                        ...
                        if abs(cx0 - table.bbox[0]) < 5:
                            cx0 = t_bbox.x0
                        if abs(cx1 - table.bbox[2]) < 5:
                            cx1 = t_bbox.x1
                        stretched_cell = (cx0, cy0, cx1, cy1)

                        ref = (
                            raw_extract[ri][orig_ci] if raw_extract[ri][orig_ci] else ""
                        )
                        row_cells.append(
                            _extract_cell_text(
                                page,
    ...
    return final_results, final_bboxes, None
